...
class SNUNet_ECAM(nn.Module):
    ...
    def forward(self, xA, xB):
        ...
        x3_1 = self.conv3_1(torch.cat([x3_0A, x3_0B, self.Up4_0(x4_0B)], 1)) #self.conv3_1 = conv_block_nested(filters[3] * 2 + filters[4], filters[3], filters[3])
...
class Siam_UNet_Conc(nn.Module):
    # SNUNet-CD without Attention
    def __init__(self, in_ch=3, out_ch=2):
        super(Siam_UNet_Conc, self).__init__()
        torch.nn.Module.dump_patches = True
        n1 = 32     # the initial number of channels of feature map
        filters = [n1, n1 * 2, n1 * 4, n1 * 8, n1 * 16]
